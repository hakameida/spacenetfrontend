...
import { selectStorageListList, StorageItem } from "@/data-access/slices/storage-list";
import StorageList from "@/feature/storage-list";
...
const useExtractedFilters = (storages: StorageItem[]) => {
  return useMemo(() => {
    ...
    storages.forEach((storage) => {
      if (storage.type_name) types.add(storage.type_name);
      if (storage.brand) brands.add(storage.brand);
      if (storage.capacity) capacities.add(storage.capacity);
    });

    return {
      types: Array.from(types).sort(),
      brands: Array.from(brands).sort(),
      capacities: Array.from(capacities).sort((a, b) => {
...
const sortProductsByPrice = (products: StorageItem[], direction: "asc" | "desc") => {
  ...
  return (
    <div className="border-b border-gray-200 py-3">
      <button
        ...
        className="flex items-center justify-between w-full text-left font-semibold text-gray-700 hover:text-cyan-600 transition-colors"
      >
...
export const AllStoragePage = ({ title }: { title: string }) => {
  ...
  const { isLoading } = useGetStoragesListQuery({ status: true });
  ...
  const storageList: StorageItem[] = Array.isArray(storageListRaw) ? storageListRaw : [];
  const sortedStorageList = sortProductsByPrice(storageList, sortDirection);
  
  const availableFilters = useExtractedFilters(sortedStorageList);
  ...
  const filteredProductList = sortedStorageList.filter((product) => {
  ...
  const FilterChip = ({ label, onRemove }: { label: string; onRemove: () => void }) => (
    <span className="inline-flex items-center gap-1 px-3 py-1 bg-gradient-to-r from-cyan-500 to-cyan-600 text-white text-sm rounded-full">
      {label}
      <button onClick={onRemove} className="hover:bg-cyan-700 rounded-full p-0.5">
        <FiX size={14} />
      </button>
    </span>
  );
  ...
  const FilterContent = () => (
    <div className="space-y-1">
      ...
      <FilterSectionWithShowMore 
        sectionKey="capacity"
        title="السعة" 
        options={availableFilters.capacities} 
        selectedValues={filters.capacity}
        onToggle={(value) => handleFilterToggle("capacity", value)}
        showAll={showAllState.capacity}
        onToggleShowAll={handleToggleShowAll}
        defaultShowCount={5}
      />
    </div>
  ...
  return (
    <>
      <div className="my-[10px] flex items-center justify-between">
        <h2 className="md:text-[34px] text-[20px] font-bold bg-gradient-to-r from-cyan-600 to-blue-600 bg-clip-text text-transparent">
          {title}
        </h2>
      </div>
      ...
      <div className="sticky top-0 z-30 bg-white/95 backdrop-blur-sm py-3 mb-4">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="flex-1 relative">
            <input
              type="text"
              placeholder="ابحث عن وحدات تخزين..."
              value={filters.manualSearch}
              onChange={(e) => handleManualSearch(e.target.value)}
              className="w-full pl-9 sm:pl-11 pr-8 sm:pr-10 py-2.5 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none transition-all"
            />
          ...
          >
            ...
            {activeFiltersCount > 0 && (
              <span className="absolute -top-2 -right-2 w-5 h-5 sm:w-6 sm:h-6 rounded-full text-xs flex items-center justify-center font-bold bg-cyan-600 text-white">
                {activeFiltersCount}
              </span>
            )}
          </button>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3">
            ...
            <div className="flex flex-wrap gap-1.5 sm:gap-2">
              ...
              {filters.capacity.map(capacity => (
                <FilterChip key={capacity} label={capacity} onRemove={() => handleFilterToggle("capacity", capacity)} />
              ))}
        ...
        >
          ...
          >
            ...
            <div className="p-4 border-t flex gap-3 flex-shrink-0 bg-white">
              ...
              <button
                onClick={() => setShowMobileFilters(false)}
                className="flex-1 py-3 bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl font-semibold transition-all"
              >
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1">
          <div className="mb-4 text-sm text-gray-600">
            عرض <span className="font-bold text-cyan-600">{filteredProductList.length}</span> من أصل{" "}
            <span className="font-bold">{storageList.length}</span> منتج
          </div>
