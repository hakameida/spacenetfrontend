...
import { selectCameraListList, CameraItem } from "@/data-access/slices/camera-list";
import CameraList from "../camera-list";
import { FiX, FiSearch, FiChevronDown, FiChevronUp, FiSliders } from "react-icons/fi";

interface FilterState {
  types: string[];      // نوع الكاميرا (DSLR, Mirrorless, Compact, Action)
  brands: string[];     // الماركة (Canon, Nikon, Sony, etc.)
  minPrice: string;
  maxPrice: string;
  manualSearch: string;
}
...
const useExtractedFilters = (cameras: CameraItem[]) => {
  return useMemo(() => {
    const types = new Set<string>();
    const brands = new Set<string>();

    cameras.forEach(camera => {
      if (camera.type_name) types.add(camera.type_name);
      if (camera.brand) brands.add(camera.brand);
    });
...
const sortProductsByPrice = (products: CameraItem[], direction: "asc" | "desc") => {
...
export const AllCameraPage = ({ title }: { title: string }) => {
  ...
  const { isLoading } = useGetCamerasListQuery({ status: true });
  ...
  const cameraList: CameraItem[] = Array.isArray(cameraListRaw) ? cameraListRaw : [];
  const sortedCameraList = sortProductsByPrice(cameraList, sortDirection);
  
  const availableFilters = useExtractedFilters(sortedCameraList);
  ...
  const filteredProductList = sortedCameraList.filter((product) => {
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1">
          <div className="mb-4 text-sm text-gray-600">
            ...
            <span className="font-bold">{cameraList.length}</span> كاميرا
          </div>
