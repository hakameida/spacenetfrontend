...
import { selectPlayStationListList, PlayStationItem } from "@/data-access/slices/playstation-list";
import PlayStationList from "../playstation-list";
import { FiX, FiSearch, FiChevronDown, FiChevronUp, FiSliders } from "react-icons/fi";

interface FilterState {
  types: string[];      // نوع البلايستيشن (PS4, PS5, PS4 Pro, etc.)
  minPrice: string;
  maxPrice: string;
  manualSearch: string;
}
...
const useExtractedFilters = (playstations: PlayStationItem[]) => {
  return useMemo(() => {
    const types = new Set<string>();

    playstations.forEach(playstation => {
      if (playstation.type_name) types.add(playstation.type_name);
    });
...
const sortProductsByPrice = (products: PlayStationItem[], direction: "asc" | "desc") => {
...
export const AllPlayStationPage = ({ title }: { title: string }) => {
  ...
  const { isLoading } = useGetPlayStationsListQuery({ status: true });
  ...
  const playstationList: PlayStationItem[] = Array.isArray(playstationListRaw) ? playstationListRaw : [];
  const sortedPlayStationList = sortProductsByPrice(playstationList, sortDirection);
  
  const availableFilters = useExtractedFilters(sortedPlayStationList);
  ...
  const filteredProductList = sortedPlayStationList.filter((product) => {
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1">
          <div className="mb-4 text-sm text-gray-600">
            ...
            <span className="font-bold">{playstationList.length}</span> بلايستيشن
          </div>
...
export default AllPlayStationPage;