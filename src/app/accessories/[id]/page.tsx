...
export default function AccessoryDetailPage({ params }: { params: { id: string } }) {
  ...
  // Get all images array - FIXED: properly typed as string[]
  ...
  // Handle swipe gestures - FIXED: added null checks
  ...
  // Handle next/previous buttons - FIXED: added null checks
  const nextImage = () => {
    const currentIndex = allImages.findIndex(img => img === currentImage);
  ...
  const handleShare = async () => {
    ...
    const shareText = `🎧 *${accessory.name}*\n\n💰 السعر: $${formatPriceInUSD(accessory.price)}\n📦 الحالة: ${getAgeInArabic(accessory.age)}${specsText}\n\n🔗 الرابط:`;
  ...
  const handleWhatsAppOrder = () => {
    ...
    message += `💰 السعر: $${formatPriceInUSD(accessory.price)}\n`;
    message += `📦 الحالة: ${getAgeInArabic(accessory.age)}`;
    message += specsText;
              ...
              <div className="flex items-baseline justify-between flex-wrap gap-2">
                <div>
                  ...
                  <p className="text-2xl md:text-3xl font-bold text-blue-600">
                    {formatPriceInSYP(accessory.price, dollar)} <span className="text-sm">ل.س</span>
                  </p>
                </div>
