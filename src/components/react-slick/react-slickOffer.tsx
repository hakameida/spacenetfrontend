...
interface MultipleItemsOfferProps {
  productModule?: ProductModule;  // If provided, filters offers by this module
  showAll?: boolean;              // If true, shows all offers regardless of productModule
  limit?: number;
}
...
const getModuleColor = (module: ProductModule) => {
  switch (module) {
    case 'LAPTOP': return 'bg-blue-600';
    case 'COMPUTER': return 'bg-green-600';
...
const OfferCard = ({ offer }: { offer: Offer }) => {
  ...
  }, [offer.productModule, laptopData, computerData, accessoryData, playstationData, cameraData]);
            ...
            <div className="grid grid-cols-2 gap-2 mt-3 text-sm">
              {product.type_name && (
                <div className="bg-gray-50 rounded p-2">
                  ...
                  <p className="text-xs font-medium text-gray-800">{product.type_name}</p>
                </div>
              )}
              {product.storage && (
                <div className="bg-gray-50 rounded p-2">
                  <p className="text-xs text-gray-500">المساحة</p>
                  <p className="text-xs font-medium text-gray-800">{product.storage}</p>
                </div>
              )}
              {product.color && (
                <div className="bg-gray-50 rounded p-2">
                  <p className="text-xs text-gray-500">اللون</p>
                  <p className="text-xs font-medium text-gray-800">{product.color}</p>
                </div>
              )}
            </div>
          )}
          ...
          {offer.productModule === 'CAMERA' && (
            <div className="grid grid-cols-2 gap-2 mt-3 text-sm">
              {product.brand && (
                <div className="bg-gray-50 rounded p-2">
                  <p className="text-xs text-gray-500">الماركة</p>
                  <p className="text-xs font-medium text-gray-800">{product.brand}</p>
                </div>
              )}
              {product.megapixels && (
                <div className="bg-gray-50 rounded p-2">
                  <p className="text-xs text-gray-500">الدقة</p>
                  <p className="text-xs font-medium text-gray-800">{product.megapixels}</p>
                </div>
              )}
              {product.sensorType && (
                <div className="bg-gray-50 rounded p-2">
                  <p className="text-xs text-gray-500">نوع المستشعر</p>
                  <p className="text-xs font-medium text-gray-800">{product.sensorType}</p>
                </div>
              )}
            </div>
          )}
...
function MultipleItemsOffer({ productModule, showAll = false, limit = 10 }: MultipleItemsOfferProps) {
  const sliderRef = useRef<Slider>(null);
  const { data, isLoading, error } = useGetOffersListQuery({});
  const allOffers: Offer[] = data || [];
  ...
  if (showAll) {
    // Show all offers regardless of module
    filteredOffers = allOffers;
  } else if (productModule) {
    // Show only offers for the specific module
    filteredOffers = allOffers.filter(offer => offer.productModule === productModule);
  } else {
